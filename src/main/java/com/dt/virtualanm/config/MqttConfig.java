package com.dt.virtualanm.config;
...
import com.dt.virtualanm.messaging.MqttMessageReceiver;
...
public class MqttConfig {
	...
	@Bean
	public MessageChannel anmInputChannel() {
	...
	public MessageProducer inbound() {
		adapter = new MqttPahoMessageDrivenChannelAdapter(clientId, mqttClientFactory(),
				"anm.test.temperature", "anm.test.pressure");
		adapter.setCompletionTimeout(5000);
		adapter.setConverter(new DefaultPahoMessageConverter());
		adapter.setQos(qos);
		adapter.setOutputChannel(anmInputChannel());
		return adapter;
	}

	@Bean
	@ServiceActivator(inputChannel = "anmInputChannel")
