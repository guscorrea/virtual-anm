...
public class TemperatureRepository {
	...
	public List<Temperature> findAllByAnmId(UUID anmId) {
		final ResultSet result = session.execute(select().all().from(TABLE).where(eq("anm_id", anmId)));
		return mapper.map(result).all();
	}

	public List<Temperature> findAllByAnmIdAndDateTime(UUID anmId, String startDateTime, String endDateTime) {
		final ResultSet result = session.execute(
				select().all().from(TABLE).where(eq("anm_id", anmId))
